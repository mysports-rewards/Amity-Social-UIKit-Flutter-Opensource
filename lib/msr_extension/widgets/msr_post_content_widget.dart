...
import 'package:amity_uikit_beta_service/view/social/image_viewer.dart';
...
class MsrAmityPostContentWidget extends StatefulWidget {
  ...
  const MsrAmityPostContentWidget(
    this.posts,
    this.isChildrenPost,
  ...
  @override
  MsrAmityPostContentWidgetState createState() => MsrAmityPostContentWidgetState();
}

class MsrAmityPostContentWidgetState extends State<MsrAmityPostContentWidget> {
  ...
  Widget build(BuildContext context) {
    if (!widget.isChildrenPost) {
      if (widget.posts[0].children != null && urlValidation(widget.posts[0])) {
        ...
        String url = extractLink(widget.posts[0]); //urlExtraction(widget.posts[0]);

        return Column(
          children: [
            // Text(url),
            widget.shouldShowTextPost ? TextPost(post: widget.posts[0], feedType: widget.feedType) : Container(),

            !urlValidation(widget.posts[0]) ? const SizedBox() : CustomLinkPreview(url: url.toLowerCase())
  ...
  Widget _buildMediaGrid(List<AmityPost> files) {
    ...
    Widget backgroundImage(String fileUrl, int index, {BorderRadius? borderRadius}) {
      return Padding(
        padding: const EdgeInsets.all(2.0),
        child: Container(
          padding: const EdgeInsets.all(2.0),
    ...
    Widget buildSingleImage(List<AmityPost> files) {
      return AspectRatio(
        ...
        child: GestureDetector(
          ...
          child: backgroundImage(getURL(files[0].data!), 0, borderRadius: BorderRadius.circular(8)),
        ),
      );
    }
    ...
    Widget buildDefaultImage(List<AmityPost> files) {
      return AspectRatio(
        ...
        child: Container(
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(8.0), // Border radius for the entire grid
        ...
        child: Stack(
          children: [
            ListTile(
              ...
              contentPadding: const EdgeInsets.symmetric(vertical: 8, horizontal: 14), // Reduced padding
...
class _TextPostState extends State<TextPost> {
  ...
  List<TextSpan> _buildTextSpans(String text, TextStyle? textStyle, TextStyle? linkStyle) {
    ...
    return elements.map((element) {
      if (element is LinkableElement) {
        return TextSpan(
          text: element.text,
          style: linkStyle,
          recognizer: TapGestureRecognizer()..onTap = () => _onOpenLink(element),
        );
      } else {
        return TextSpan(
          text: element.text,
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle? textStyle = context.msrMaterialTheme.textTheme.bodyMedium?.copyWith(
      color: context.msrMaterialTheme.colorScheme.onSurface,
    );
    TextStyle? linkStyle = context.msrMaterialTheme.textTheme.bodyMedium?.copyWith(
      color: context.msrMaterialTheme.colorScheme.onSurface,
      decoration: TextDecoration.underline,
      decorationColor: context.msrMaterialTheme.colorScheme.onSurface,
    );
                      ...
                      ? Padding(
                          ...
                          child: shouldShorten
                              ? RichText(
                                  text: TextSpan(
                                    children: [
                                      ..._buildTextSpans(text.substring(0, 180), textStyle, linkStyle),
                                      TextSpan(
                                        text: " ... Load more",
                                        style: linkStyle,
                              ...
                              : GestureDetector(
                                  ...
                                  child: RichText(
                                    text: TextSpan(
                                      children: _buildTextSpans(text, textStyle, linkStyle),
                                    ),
                                  ),
                                ),
                        )
...
class ImagePost extends StatelessWidget {
  ...
  final bool isCornerRadiusEnabled;
  const ImagePost({Key? key, required this.posts, required this.imageURLs, required this.isCornerRadiusEnabled})
      : super(key: key);
          ...
          builder: (BuildContext context) {
            return GestureDetector(
              ...
              child: Container(
                ...
                margin: EdgeInsets.symmetric(horizontal: imageURLs.length > 1 ? 5.0 : 0.0),
                ...
                child: ClipRRect(
                    ...
                    child: FadeInImage(
                      ...
                      fit: BoxFit.cover,
                      placeholder: const AssetImage('assets/images/placeholder.png'), // Local asset for placeholder
                    )),
              ),
            );
          },
  ...
  Route _goToImageViewer(String url) {
    return PageRouteBuilder(
      pageBuilder: (context, animation, secondaryAnimation) =>
          ImageViewer(imageURLs: imageURLs, initialIndex: imageURLs.indexOf(url)),
